...
export function TrendingCategories({ title = "Trending on eBay", data = [] }) {
        ...
        <section className="mb-12 cursor-pointer">
            ...
            <div className="flex overflow-x-auto pb-6 gap-4 snap-x">
                {data.map((item) => {
                    ...
                    const image = isProduct ? item.thumbnail : (item.imageUrl || item.iconUrl);
                    const link = isProduct ? `/products/${item.id}` : `/products?category=${item.slug}`;
