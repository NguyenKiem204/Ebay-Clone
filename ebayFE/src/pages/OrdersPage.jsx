                                ...
                                <div key={order.id} className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                                    <div className="px-6 py-4 bg-gray-50 border-b border-gray-200 flex flex-wrap justify-between items-center gap-4">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <p className="text-sm font-mono font-bold text-gray-900">#{order.id.toString().padStart(8, '0')}</p>
                                        </div>
                                    </div>

                                    <div className="p-6">
                                        {order.items?.map(item => (
                                            <div key={item.id} className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6 last:mb-0">
                                                ...
                                                <div className="flex flex-col space-y-2 w-full sm:w-48 shrink-0 mt-4 sm:mt-0">
                                                    {(order.status?.toLowerCase() === 'shipped' || order.status?.toLowerCase() === 'processing') && (
                                                        <Button variant="primary" size="sm" className="w-full font-bold shadow-sm">Track package</Button>
                                                    )}
                                                    {order.status?.toLowerCase() === 'delivered' && (
                                                        <Button variant="primary" size="sm" as={Link} to={`/products/${item.productId}`} className="w-full font-bold shadow-sm">Buy again</Button>
                                                    )}
                                                    <Button variant="outline" size="sm" as={Link} to={`/orders/${order.id}`} className="w-full font-bold">View details</Button>
                                                    {order.status?.toLowerCase() === 'pending' && (
                                                        <Button variant="ghost" size="sm" className="w-full font-bold text-gray-500 hover:text-red-600" onClick={() => handleCancel(order.id)}>Cancel order</Button>
                                                    )}
                                                </div>
                                            </div>
                                        ))}
